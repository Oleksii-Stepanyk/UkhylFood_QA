...
public class RestaurantOrderConsumer : IConsumer<Order>
{
    private readonly ICourierService _courierService;
    private readonly ILogger<RestaurantOrderConsumer> _logger;
    private readonly Tracer _tracer;

    public RestaurantOrderConsumer(ICourierService courierService, ILogger<RestaurantOrderConsumer> logger, Tracer tracer)
    {
    ...
    public async Task Consume(ConsumeContext<Order> context)
    {
        ...
        _logger.LogDebug("Received order with ID {OrderId} and status {Status}", order.Id, order.Status);
